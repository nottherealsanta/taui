...
Taui is a tool for building software through structured intent. Instead of writing code directly or giving freeform chat instructions, users and agents co-author **tangles**: living documents that describe what the system should do, grounded in dense references to the actual code. The tangle is the source of truth; code is the derived artifact.
...
- **Tangle format**: Only `title` and `last_updated` required in frontmatter. Everything else is body content shaped by the tangle-maker prompt.
- **All paths relative to project root**: `src/auth.py` means `<project>/src/auth.py`. `tangles/auth.md` means `<project>/tangles/auth.md`.
- **Backend is the single source of truth**: UI holds no authoritative state. All persistent state lives in the backend DB or `settings.json`.
- **Project-local storage**: `tangles/.taui.db` (gitignored) for runtime/derived data; `.taui/settings.json` (optionally tracked) for user settings.
- **User-editable prompts**: Agent system prompts are not hardcoded — they ship with defaults but users can view and edit them in Settings.
...
1. **Frontend** (`app/`) — Svelte 5 + Tauri. Obsidian-like three-column layout: left nav, center editor, right agent pane. Stateless — all persistent state comes from backend via WebSocket.

2. **Backend** (`taui/`) — Python/FastAPI. WebSocket JSON-RPC server. Manages tangle files, agent sessions, and all state. Reads/writes `tangles/.taui.db` for runtime data and `.taui/settings.json` for user preferences.

3. **Storage** — Two project-local stores:
