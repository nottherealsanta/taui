...
- **CLI** (default) — prompt-toolkit REPL. This is what starts when you run `taui`.
- **TUI** (opt-in) — Textual terminal UI with panes, scrollable history, visual tool output.
- **Web** (opt-in) — Svelte frontend backed by FastAPI, Monaco editor, diff views.

All three share the same protocol and Store, so anything built for one works with any. The defaults are intentionally bare. The goal is not to lock you into one workflow, but to give you the pieces to build your own.
...
- **Extensions are isolated.** A broken extension (bad tool, crashing UI component, malformed prompt) does not take down the core agent loop, the Store, or the CLI. The base system always remains functional.
...
- UI customization can change layout, panes, and interaction patterns instead of forcing a fixed shell.
...
- Single-process runtime is the default: agent loops and web transport run in one Python process to keep streaming and coordination simple. Components are modular with explicit interfaces, so they can be separated into distinct processes later without redesigning boundaries.
