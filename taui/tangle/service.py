...
class SpecService:
    ...
    def __init__(
        ...
        specs_dir: str = "tangles",
        dev_mode: bool = False,
    ) -> None:
        self.workspace = Path(workspace or Path.cwd()).resolve()
        root_path = tangles_path if tangles_path is not None else specs_path
        if root_path is None:
            tangles_candidate = (self.workspace / "tangles").resolve()
            specs_candidate = (self.workspace / "specs").resolve()
            if specs_candidate.exists() and not tangles_candidate.exists():
                self.spec_root = specs_candidate
            else:
        ...
        self._ensure_gitignore_db_entry()
    ...
    def _ensure_gitignore_db_entry(self) -> None:
        gitignore_path = self.workspace / ".gitignore"
        entry = "tangles/.taui.db"
        try:
            ...
            logger.debug("Failed to ensure %s in .gitignore", entry)
    ...
    async def get_tree(self) -> list[SpecNode]:
        ...
        use_v2 = self.spec_root.name == "tangles"
        v2_rows = await self.db.get_tangle_tree() if use_v2 else []
        if v2_rows:
            nodes: list[SpecNode] = []
            for row in v2_rows:
