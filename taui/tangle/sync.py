...
class SpecSync:
    ...
    async def full_sync(self) -> None:
        ...
        # Populate tangle-v2 tables in parallel to legacy tables.
        await self.db._execute("DELETE FROM tangle_links")
        for path in files:
            rel_path = self._to_rel_path(path)
            text = path.read_text(encoding="utf-8")
            content_hash = sha256(text.encode("utf-8")).hexdigest()
            mtime_ns = path.stat().st_mtime_ns
            detail = parse_tangle_document(
                rel_path,
                text,
            ...
            file_id = await self.db.upsert_tangle_file(
                rel_path=detail.file.rel_path,
                content_hash=detail.file.content_hash,
                mtime_ns=detail.file.mtime_ns,
                title=detail.file.title,
                last_updated=detail.file.last_updated,
                now_ts=now_ts,
            )
            ...
            refs_payload: list[dict[str, object]] = []
            for ref in detail.refs:
                owner_id = ""
                for node in detail.nodes:
                    if node.line_start <= ref.line_in_tangle <= node.line_end:
                        owner_id = node.id
                        break
                if not owner_id and detail.nodes:
                    owner_id = detail.nodes[0].id
                if not owner_id:
                    continue
                refs_payload.append(
                    {
                        ...
                        "target": ref.target,
                        "context": ref.context,
                        "line_in_tangle": ref.line_in_tangle,
                    }
                )
            ...
            await self.db.replace_tangle_nodes(
                file_id=file_id,
                nodes=nodes_payload,
                refs=refs_payload,
                links=links_payload,
        ...
        await self.db.delete_missing_tangle_files(set(parsed.keys()))
    ...
    def _parse_nodes_standard(
        ...
        *,
        ...
        root_anchor = (
            slugify(fm_title)
            if fm_title
            else slugify(rel_path.split("/")[-1].replace(".md", ""))
        )
