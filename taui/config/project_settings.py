...
def default_prompt_content() -> dict[str, str]:
    return {
        ...
        "tangle_maker": (
            ...
            "- `## Key Components` — each component described in 1-2 sentences with code ref\n"
            "- `## Code References` — dense list of all important symbols with refs\n"
            ...
            "- `## Design` — architecture, data flow, key decisions, with inline code refs\n"
            "- `## Code References` — all important symbols\n"
            ...
            "## Writing Guidelines\n\n"
            "1. **Dense code refs**: Every mention of a class, function, method, or constant "
            "should include a code reference. The UI renders these inline — they are the "
            "bridge between prose and implementation.\n"
            "2. **Explain intent, not mechanics**: Describe *why* and *what*, not *how*. "
            "The code ref shows the how.\n"
            "3. **State constraints directly**: 'All file paths must be relative to project "
            "root' — not 'we generally prefer relative paths'.\n"
            "4. **Keep files medium-sized**: Long enough to fully explain one domain or "
            "feature. Short enough to be a useful context packet for an agent.\n"
            "5. **Never duplicate code**: Reference it. The UI renders it inline.\n"
            "6. **Separate fact from inference**: Distinguish known constraints from open "
            "questions. Use the Open Questions section.\n"
            "7. **Link outward**: Every tangle should link to related tangles, code, and "
