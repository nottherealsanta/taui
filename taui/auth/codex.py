...
CLIENT_ID = "app_EMoamEEZ73f0CkXaXp7hrann"  # plaintext, not base64
...
def _get_account_id(access_token: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"chatgpt_account_id not found in JWT payload") from exc
...
def login() -> CodexCredentials:
    ...
    state = _secrets.token_bytes(
        16
    ).hex()  # separate from verifier, unlike Google providers
...
def get_codex_credentials() -> CodexCredentials:
    ...
    saved = config.load_provider_config("codex")
    if saved:
        refresh_token = saved.get("refresh_token")
        account_id = saved.get("account_id", "")
        if refresh_token:
            try:
                # Use an expired expires_at_ms to force a refresh on first use
