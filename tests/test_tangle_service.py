...
from taui.tangle import SpecNotFoundError, SpecService
...
def test_get_tree_uses_custom_specs_path(tmp_path: Path) -> None:
    ...
    service = SpecService(workspace=tmp_path, specs_path="example_project/tangles")
    refs = {node.spec_ref for node in _run(service.get_tree())}
    assert "example_project/tangles/main.md#project-spec" in refs
