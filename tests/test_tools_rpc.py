...
def test_rpc_ui_node_edited_applies_edit(tmp_path: Path) -> None:
    """ui/nodeEdited updates the tangle node and emits tangle/nodeChanged notification."""
    _write_specs(tmp_path)
    app = create_app(workspace=tmp_path)

    with TestClient(app) as client:
        with client.websocket_connect("/ws") as ws:
            # First, get the tree to find a valid tangle_ref
            tree_resp = _rpc(ws, 1, "tangle/getTree", {})
            assert "error" not in tree_resp, tree_resp
            nodes = tree_resp["result"]["nodes"]
            assert len(nodes) > 0

            # Pick the first node — use spec_ref as the handler accepts it as fallback
            tangle_ref = nodes[0].get("tangle_ref") or nodes[0].get("spec_ref")
                ...
                json.dumps(
                    {
                        ...
                        "params": {
                            ...
                            # Keep same leading heading to avoid tangle_ref change
